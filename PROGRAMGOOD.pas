...
var
    eps, d, q, x_real: real;
    i, j, amount_of_points, char_after_point, n, x1_integer, amount_of_digits, 
    t, length_of_palindrome, g, amount_of_minuses, start, finish,
    max_length_pal, k, rank: integer;
    task_a, task_b, det_arith_progres, det_geom_progres, det_palindrome, 
    point, det_negative, spaces_after_num, det_error, result, flag: boolean;
    mas: array of real;
...
begin
    ...
    begin
       ...
       t:=0;
        for i:=1 to n-1 do
        begin
           if (abs(mas[i]-mas[g]) <= eps) and (g = 0) then
           begin
               g := 1;
               t := i;
           end
